...
async function loadProducts() {
    try {
        ...
        products = data.map(product => ({
            ...
            in_stock: product.in_stock !== 0 // Kiểm tra hàng còn hay không
        }));
...
function renderProducts() {
    ...
    productGrid.innerHTML = filteredProducts.map(product => {
        ...
        console.log(`Product: ${product.name}, in_stock value:`, product.in_stock, 'isInStock:', isInStock);
        return `
        <div class="product-card" data-product-id="${product.id}">
            <div class="product-image-wrapper">
                <img src="${product.image}" alt="${product.name}" class="product-image" style="${!isInStock ? 'opacity: 0.6; filter: grayscale(30%);' : ''}">
                ${product.badge ? `<div class="product-badge">${product.badge}</div>` : ''}
                ${!isInStock ? '<div class="product-badge" style="background: #ef4444;">Hết hàng</div>' : ''}
            </div>
            <div class="product-info">
                ...
                <div class="product-footer">
                    ...
                    <button class="add-to-cart-btn" data-product-id="${product.id}" ${!isInStock ? 'disabled' : ''} style="${!isInStock ? 'opacity: 0.5; cursor: not-allowed; pointer-events: none;' : ''}">
    ...
    document.querySelectorAll('.add-to-cart-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (btn.disabled) {
                alert('⚠️ Sản phẩm hiện đang hết hàng');
                return;
            }
